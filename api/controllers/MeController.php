...
function media_url(?string $rel): ?string {
  if (!$rel) return null;
  $clean = ltrim(str_replace('\\','/', (string)$rel), '/');
...
class MeController {
  public static function myDesigns(): void {
    ...
    $sql = 'SELECT * FROM designs WHERE '.implode(' AND ',$w).' ORDER BY '.$order.' LIMIT '.$limit.' OFFSET '.$offsetInt;
    $st  = $pdo->prepare($sql);
    $st->execute($p);
    $rows = $st->fetchAll();
  ...
  public static function saved(): void {
    ...
    $sql = 'SELECT d.* FROM design_saves s JOIN designs d ON d.id=s.design_id
            WHERE '.implode(' AND ', $w).'
            ORDER BY '.$order.' LIMIT '.$limit.' OFFSET '.$offsetInt;
    $st = $pdo->prepare($sql);
    $st->execute($p);
    $rows = $st->fetchAll();
