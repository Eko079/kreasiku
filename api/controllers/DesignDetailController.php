...
class DesignDetailController {
  public static function show(int $id): void {
    ...
    // Ambil desain + izin
    $sql = "SELECT d.*, u.name AS owner_name, u.avatar AS owner_avatar, u.avatar_url AS owner_avatar_url, u.avatar_path AS owner_avatar_path
    ...
    // ambil daftar gambar (jika ada)
    $images = Design::getImages($id);
    $media  = $images[0] ?? null;
    if (!$media && !empty($row['media_path'])) {
      $media = media_public_url($row['media_path']);
    }
    ...
    json_ok([
      'data'=>[
        ...
        'media'         => $media,
        'mediaPath'     => media_public_url($row['media_path']),
        'images'        => $images,
        'ownerId'       => (int)$row['owner_id'],
        'ownerName'     => $row['owner_name'],
        'ownerAvatar'   => media_public_url($row['owner_avatar_path'] ?? null) ?? media_public_url($row['owner_avatar'] ?? null) ?? ($row['owner_avatar_url'] ?? null),
        'liked'         => $liked,
        'saved'         => $saved,
