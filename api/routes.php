...
function resolve_route(string $method, string $path) {
  ...
  if ($scriptDir && $scriptDir !== '/' && starts_with($path, $scriptDir)) {
  ...
  // Users (profile)
  if ($method==='GET'  && $path==='/users/me')     return ['UsersController::getMe'];
  if (in_array($method, ['PATCH','POST'], true) && $path==='/users/me') return ['UsersController::updateMe'];
  ...
  // Detail route handled later by DesignDetailController
