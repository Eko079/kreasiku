...
$q = $db->prepare('SELECT c.id, c.body, c.created_at,
                          u.id AS user_id, u.name, u.email, u.avatar_url, u.avatar_path
                       FROM comments c
  ...
  $payload = array_map(function($row){
    return [
      ...
      'user_name'  => $row['name'] ?: ($row['email'] ?? 'Anonim'),
      'avatar'     => $row['avatar_url'] ?? $row['avatar_path'] ?? null,
      'body'       => $row['body'],
      'created_at' => $row['created_at'],
    ];
  }, $items);

  json_ok(['items'=>$payload, 'total'=>$total, 'page'=>$page, 'limit'=>$limit]);
} catch (Throwable $e) {
