...
/* pastikan desain ada & publik atau milik sendiri */
...
/* toggle */
$st = $db->prepare("SELECT 1 FROM design_likes WHERE design_id=:d AND user_id=:u");
$st->execute([':d'=>$design_id, ':u'=>$uid]);
$has = (bool)$st->fetchColumn();

if ($has) {
  $db->prepare("DELETE FROM design_likes WHERE design_id=:d AND user_id=:u")
     ->execute([':d'=>$design_id, ':u'=>$uid]);
  $liked = false;
} else {
  $db->prepare("INSERT INTO design_likes(design_id,user_id) VALUES(:d,:u)")
     ->execute([':d'=>$design_id, ':u'=>$uid]);
  $liked = true;
}
$count = (int)$db->query("SELECT COUNT(*) FROM design_likes WHERE design_id=".$design_id)->fetchColumn();

json_ok(['liked'=>$liked,'likes_count'=>$count]);
