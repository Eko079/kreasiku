...
class Design {
  ...
  public static function create(array $payload): int {
    ...
    $rawComments    = $payload['allow_comments'] ?? ($payload['comment_enabled'] ?? null);
    $allowComments  = $rawComments === null ? 1 : (int)!!$rawComments;
  ...
  public static function list(array $q): array {
    ...
    $sql = 'SELECT * FROM designs '.(count($w)?'WHERE '.implode(' AND ',$w):'').' ORDER BY '.$order.' LIMIT 60';
    $st = $pdo->prepare($sql);
    $st->execute($p);
    return $st->fetchAll();
  }

  /** Ambil daftar gambar untuk sebuah desain */
  public static function getImages(int $designId): array {
    global $pdo;
    $st = $pdo->prepare('SELECT file_path FROM design_images WHERE design_id = ? ORDER BY position ASC, id ASC');
    $st->execute([$designId]);
    $rows = $st->fetchAll();
    $images = [];
    ...
    foreach ($rows as $row) {
      ...
      if (str_starts_with($rel, 'storage/')) {
        ...
        $images[] = $rootBase . '/storage/' . $rel;
      }
    }
    return $images;
  }

  /** Tambah gambar untuk desain */
  public static function addImage(int $designId, string $filePath, int $position = 0): void {
    global $pdo;
    $st = $pdo->prepare('INSERT INTO design_images (design_id, file_path, position) VALUES (?, ?, ?)');
    $st->execute([$designId, $filePath, $position]);
  }
}
